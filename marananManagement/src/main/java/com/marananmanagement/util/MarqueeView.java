...
import com.marananmanagement.BuildConfig;
...
public class MarqueeView extends LinearLayout {
	...
	private void extractAttributes(AttributeSet attrs) {
		...
		TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.asia_ivity_android_marqueeview_MarqueeView);
		...
		mSpeed = a.getInteger(
				R.styleable.asia_ivity_android_marqueeview_MarqueeView_speed,
				DEFAULT_SPEED);
		mAnimationPause = a.getInteger(
				R.styleable.asia_ivity_android_marqueeview_MarqueeView_pause,
				DEFAULT_ANIMATION_PAUSE);
		mAutoStart = a
				.getBoolean(
						R.styleable.asia_ivity_android_marqueeview_MarqueeView_autoStart,
						false);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed) {
			View v = getChildAt(0);
			// Fixes #1: Exception when using
			// android:layout_width="fill_parent". There seems to be an
			// additional ScrollView parent.
			if (v instanceof ScrollView
					&& ((ScrollView) v).getChildCount() == 1) {
